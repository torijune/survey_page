...
export default function SurveyListPage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      router.push(`/manage/surveys/${survey.id}/edit`);
    } catch (e: any) {
      setError(e.message);
      setCreating(false);
    }
  ...
  const getSurveyUrl = useCallback((survey: Survey) => {
    return `${window.location.origin}/survey/${survey.share_id}`;
  }, []);
  ...
  const handleShowQR = (survey: Survey) => {
    setQrSurvey(survey);
  ...
  const handleDownloadQR = () => {
    if (!qrRef.current || !qrSurvey) return;

    const svg = qrRef.current.querySelector('svg');
    if (!svg) return;

    // SVG를 Canvas로 변환 후 PNG 다운로드
    ...
    img.onload = () => {
      ...
      const a = document.createElement('a');
      a.href = pngUrl;
      a.download = `${qrSurvey.title || 'survey'}_QR.png`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      
      setSnackbarMessage('QR 코드가 다운로드되었습니다.');
      setSnackbarOpen(true);
    };
  ...
  const handleCopyLinkFromQR = () => {
    if (!qrSurvey) return;
    const url = getSurveyUrl(qrSurvey);
    navigator.clipboard.writeText(url);
    setSnackbarMessage('링크가 복사되었습니다.');
    setSnackbarOpen(true);
  };
  ...
  const handlePdfImport = async () => {
    ...
    try {
      ...
      if (result.success && result.survey_id) {
        setTimeout(() => {
          router.push(`/manage/surveys/${result.survey_id}/edit`);
        }, 1500);
      }
    } catch (e: any) {
  ...
  return (
    <>
                    ...
                    {filteredSurveys.map((survey) => {
                      ...
                      return (
                        <TableRow
                          ...
                          onClick={() => router.push(`/manage/surveys/${survey.id}/edit`)}
                        >
                          <TableCell>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                              <Avatar
                          ...
                          <TableCell align="right" onClick={(e) => e.stopPropagation()}>
                            <Stack direction="row" spacing={0.5} justifyContent="flex-end">
                              ...
                              <Tooltip title="응답 현황">
                                <IconButton
                                  size="small"
                                  onClick={() => router.push(`/manage/surveys/${survey.id}/responses`)}
                                  sx={{ color: 'grey.500', '&:hover': { color: 'primary.main' } }}
                                >
      ...
      >
        <MenuItem onClick={() => router.push(`/manage/surveys/${menuAnchor?.survey.id}/preview`)}>
        ...
        {menuAnchor?.survey.status === 'published' && (
          <MenuItem onClick={() => handleShowQR(menuAnchor.survey)}>
            <QrCode2 fontSize="small" sx={{ mr: 1.5, color: 'grey.600' }} /> QR 코드
          </MenuItem>
        )}
        <Divider sx={{ my: 1 }} />
          ...
          {qrSurvey && (
            <>
              ...
              >
                ...
                >
                  {getSurveyUrl(qrSurvey)}
                </Typography>
                <Tooltip title="링크 복사">
                  <IconButton size="small" onClick={handleCopyLinkFromQR}>
